...
Class Take_exam extends Admin_Controller {
    ...
    */
    ...
    public function exam($id) {
        ...
        $this->data['usertypeID']   = $usertypeID;
        $query = $this->db->get();
        $this->db->select()->from("question_group")->where('questionGroupID',$id);
		$query = $this->db->get();
        $examGroup = $query->row();
        $this->data['examGroup']    = $examGroup;
        $this->data['onlineExams']  = $this->online_exam_m->get_order_by_online_exam_specific_group(array('usertypeID'=>$usertypeID, 'published'=>1),$id);
        ...
        if($_POST) {
            ...
            if($this->form_validation->run() == FALSE) {
                ...
                if(isset($this->post_data['onlineExamID'])) {
                    ...
                    $this->session->set_flashdata('error', 'Exam does not found');
                    redirect(base_url('take_exam/exam'));
                }
            }
        } else {
            $this->data["subview"] = "online_exam/take_exam/exam";
            $this->load->view('_layout_main', $this->data);
        }
    }
    ...
    public function easy() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $onlineExamID   = $_POST['onlineExamID'];
            ...
            // $onlineExamID = $examtime;
        $question    =  $this->online_exam_question_m->get_order_by_online_exam_question(['onlineExamID' => $onlineExamID]);
        
        $questionID  = $question[0]->questionID;
        $onlineExamQuestionID =$question[0]->onlineExamQuestionID;
        ...
        $onlineExamUserAnswerID = $this->online_exam_user_answer_m->insert([
            ...
            $this->db->select()->from("online_exam_question")->where('onlineExamID',$onlineExamID);
            $query = $this->db->get();
            $onlineExam = $query->row();
            $questionBank = $this->question_bank_m->get_order_by_question_bank(['questionBankID'=>$onlineExam->questionID]);
            $onlineExamData = $this->online_exam_m->get_order_by_online_exam(['onlineExamID'=>$onlineExam->onlineExamID]);

            $imgArray = array();
            if(inicompute($_FILES['image'])) { 
                $imgArray[] = 'image';
            }

            if(inicompute($_FILES['file'])) { 
                $imgArray[] = 'file';
            }
            ...
            $this->online_exam_user_status_m->insert([
                'onlineExamID' => $onlineExamID,
                'time' => $time,
                'totalQuestion' => $questionBank[0]->totalQuestion,
                'totalAnswer' => 1,
                'nagetiveMark' => $onlineExamData[0]->negativeMark,
                'duration' => $onlineExamData[0]->duration,
                'score' => 0,
                'userID' => $userID,
                'classesID' => inicompute($this->data['student']) ? $this->data['student']->classesID: 0,
                'sectionID' => inicompute($this->data['student']) ? $this->data['student']->sectionID : 0,
                'examtimeID' => $examTimeCounter,
            ...
            return redirect(base_url('take_exam/4'));
        }
    }
                ...
                if($_FILES[$imgValue]['name'] !="") {
                    ...
                    if(inicompute($explode) >= 2) {
                        ...
                        if(!$this->upload->do_upload($imgValue)) {
                            ...
                            $returnArray['success'][$imgValue] = $new_file;
                        }
