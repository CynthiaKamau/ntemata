...
*/
    ...
    protected function rules()
    {
        $rules = [
            ...
            [
                'field' => 'sectionID[]',
                'label' => $this->lang->line("class_stream"),
                'rules' => 'trim|required|max_length[200]|xss_clean'
            ],
            [
                'field' => 'subjectID[]',
                'label' => $this->lang->line("select_class_stream"),
                'rules' => 'trim|required|max_length[200]|xss_clean'
            ],
            [
                'field' => 'subjects_type',
                'label' => $this->lang->line("subjects_type"),
                'rules' => 'trim|max_length[200]|xss_clean'
            ],
               [
                'field' => 'subjects_code',
                'label' => $this->lang->line("subjects_code"),
                'rules' => 'trim|max_length[200]|xss_clean'
            ],
    ...
    public function add()
    {
        ...
        $this->data['class_streams'] = $this->db->query("select * from section")->result();
        $this->data['subjectlist'] = $this->db->query("select * from subject")->result();
        if ( $_POST ) {
            $rules = $this->rules();
            ...
            if ( $this->form_validation->run() == false ) {
                ...
                if ( config_item('demo') == false ) {
                 ...
                 $section_details=implode(',',$sectionID);
                 
                 //print_r($section_details);
                 $subjectID=$this->input->post("subjectID");
                $subject_details=implode(',',$subjectID);
                ...
                $array['subjects_list']        = $subject_details;
                $array['section_list']        = $section_details;
                $array['subjects_type']        = $this->input->post("subjects_type");
                $array['subjects_code']        = $this->input->post("subjects_code");
