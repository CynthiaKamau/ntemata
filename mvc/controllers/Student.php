...
*/
	...
	protected function rules() {
		$rules = array(
			...
			array(
				...
				'rules' => 'trim|numeric|max_length[11]|xss_clean|callback_unique_sectionID|callback_unique_capacity'
			),
			array(
				'field' => 'registerNO',
	...
	public function index() {
		...
		if($usertypeID == 3) {
			...
			if($classesID=='')
			{
			  $this->data['students'] =  $this->db->query("select * from student as student left join studentextend as studentextend on studentextend.studentID=student.studentID where student.active='1'")->result();
			}
			else
			{
			...
			//$this->db->query("select * from student as student left join studentextend as studentextend on studentextend.studentID=student.studentID where student.active='1'")->result()    
	...
	public function add() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$parents      = $this->parents_m->get_single_parents(array('parentsID' => $this->input->post('guargianID')));
				...
				$this->usercreatemail($parents->email, $this->input->post('username'), $this->input->post('password'));
