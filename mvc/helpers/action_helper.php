...
function display_menu($nodes, &$menu, $groupIds = [], $currentId = null)
{
    ...
    foreach ($nodes as $key => $node) {
        ...
        if (permissionChecker($node['link']) || ($node['link'] == '#' && $f)) {
            ...
            if ($node['menuName'] == 'take_exam') {
                foreach ($groupIds as $val) {
                    $menu .= '<li class="' . ($val->questionGroupID == $currentId ? 'active' : '') . '">';
                    // ($CI->lang->line('menu_'.$node['menuName']) != NULL ? $CI->lang->line('menu_'.$node['menuName']) : $node['menuName'])
                    $menu .= anchor($node['link'] . '/' . $val->questionGroupID, '<i class="fa ' . ($node['icon'] != NULL ? $node['icon'] : 'fa-home') . '"></i><span>' . 'Take ' . $val->title . '</span> ' . ($f ? $leftIcon : ''));
                    if ($f) {
                        $menu .= '<ul class="treeview-menu">';
                        display_menu($node['child'], $menu, $groupIds, $currentId);
                        $menu .= "</ul>";
                    }
                    $menu .= "</li>";
                }
            ...
            if ($node['menuName'] == 'onlineexamreport') {
                $menu .= '<li class="' . ( $currentId == 4 ? 'active' : '') . '">';
                $menu .= anchor('report/easy/4', '<i class="fa ' . ($node['icon'] != NULL ? $node['icon'] : 'fa-home') . '"></i><span>' . 'Essay Report '. '</span> ' . ($f ? $leftIcon : ''));
                $menu .= "</li>";
                $menu .= '<li class="' . ($currentId == 6 ? 'active' : '') . '">';
                $menu .= anchor('report/easy/6', '<i class="fa ' . ($node['icon'] != NULL ? $node['icon'] : 'fa-home') . '"></i><span>' . 'Reflection Report '. '</span> ' . ($f ? $leftIcon : ''));
                $menu .= "</li>";
            }

            if ($node['menuName'] != 'take_exam') {
                if($CI->session->userdata('usertypeID') == 1) {
                    if($node['menuName'] == 'Student Details'){
                        $menu .= '<li class="' . ($f ? $treeView : '') . $active . '">';
                        $menu .= anchor($node['link'], '<i class="fa ' . ($node['icon'] != NULL ? $node['icon'] : 'fa-home') . '"></i><span>' . ($CI->lang->line('menu_' . $node['menuName']) != NULL ? $CI->lang->line('menu_' . $node['menuName']) : "Manage learners") . '</span> ' . ($f ? $leftIcon : ''));
                        if ($f) {
                            $menu .= '<ul class="treeview-menu">';
                            display_menu($node['child'], $menu, $groupIds, $currentId);
                            $menu .= "</ul>";
